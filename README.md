...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/faa_regulation \
  -H 'Content-Type: application/json' \
  -d '{"citation":"91.113"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/faa_regulation`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "faa-regulations": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-faa-regulations"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-faa-regulations
```
